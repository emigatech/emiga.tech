...
$(document).ready(function() {
  ...
  if(app_query['q'] == undefined) {
  	...
  	seo_image = String(`https://dummyimage.com/1200x630/ffffff/dee2e6.png&text=`+filterXSS(app_query['q'])+`+on+emiga.tech`);
  	seo_description = String(filterXSS(app_query['q']) + ` | emiga.tech provides top technology news, with investigative reporting and in-depth coverage of tech issues and events. You know us.`);
  	seo_title = String(filterXSS(app_query['q']) + ` | emiga.tech ― You know us.`);

  	// Title
  	$(document).prop('title', seo_title);
  	$("meta[property='og:title']").attr('content', seo_title);
  	$("meta[name='twitter:title']").attr('content', seo_title);
  	$("link[rel='canonical']").attr('href', filterXSS(window.location.href));
  	...
  	// Image
  	$("meta[property='og:image']").attr("content", seo_image);
  	$("meta[property='og:image:secure_url']").attr("content", seo_image);
  	$("meta[name='twitter:image']").attr("content", seo_image);
